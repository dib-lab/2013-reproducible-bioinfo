...
would type by hand onto punch cards.  In the 1980s,
...
documentation, etc., and therefore some biologists may
select software based not on accuracy and scientific relevance, but simply on
ease of use.  

\item With the trial and error
approach used to create custom pipelines, some biologists can
...
to command line is useful because
the power available at the command line outstrips that
...
is like a lab that is equipped with chemicals and instruments that can be used 
...
system, but Bourne-again Shell (Bash) is predominant. 
...
publications so that anyone can download our source codes and materials
for using or reproducing resutls without contacting us. 
In addition, collaborators or anyone who find a bug or want to contribute to
the project can do so by simply cloning the project, editing or adding the code
and submitting a request to merge a change to the project.
This opens an opportunity for improving a quality of scientific software
as well as collaboration.
This method has been proven very successful in opensource community.
...
languages.
For example, with rmagic plugin and Python RPy2 library, users can
use R libraries such as Bioconductors (\emph{see Resources})
and store the result in Python data structures, which can be analysed further
with Python.
This feature faciliates the analysis by reducing many steps in
data transformation, which has always been a problem in
bioinformatics research.
Another useful feature of IPython notebook is an in-line plot, which allows
users to use Matplotlib library or R to make and visualize a plot then save it
